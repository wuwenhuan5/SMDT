...
vector<float> disparityErrorsOutlier (DisparityImage &D_gt,DisparityImage &D_orig,DisparityImage &D_ipol,bool refl) {
  ...
  for (int32_t u=0; u<width; u++) {
    for (int32_t v=0; v<height; v++) {
      if (D_gt.isValid(u,v)) { //所有像素
        float d_err  = fabs(D_gt.getDisp(u,v)-D_ipol.getDisp(u,v));
        for (int32_t i=0; i<5; i++) //阈值从0到5，
          if (d_err>(float)(i+1))
            errors[i*2+0]++; //0,2,4,6,8，偶数表示GT中的所有像素
        num_pixels++;
        if (D_orig.isValid(u,v)) { //估计像素
          for (int32_t i=0; i<5; i++)
            if (d_err>(float)(i+1))
              errors[i*2+1]++; //1,3,5,7,9，奇数表示估计的有效像素
          num_pixels_result++; 
        }
      }
    }
  ...
  if (refl) {
    ...
    // push back density
    errors.push_back((float)num_pixels_result/max((float)num_pixels,1.0f)); //有效density的比例
  }
...
bool eval (string result_sha="", Mail* mail=NULL) {

  // ground truth and result directories
  string gt_noc_dir      = "KITTI_training/disp_noc";
  string gt_occ_dir      = "KITTI_training/disp_occ";
  string gt_refl_noc_dir = "KITTI_training/disp_refl_noc";
  string gt_refl_occ_dir = "KITTI_training/disp_refl_occ";
  string gt_img_dir      = "KITTI_training/image_0";
  string result_dir      = "KITTI_training/results";

  // create output directories
  /*
  system(("mkdir " + result_dir + "/errors_noc_out/").c_str()); //调用系统命令创建目录
  ...
  */
  ...
  for (int32_t i=0; i<194; i++) { //i<195
    ...
    try {
      ...
      string image_file = result_dir + "/data/" + prefix + ".png"; //存放视差图
      if (!imageFormat(image_file,png::color_type_gray,16,D_gt_noc.width(),D_gt_noc.height())) { //判断格式是否一致
      ...
      D_ipol.interpolateBackground(); //使用背景插值

      // add disparity errors
      vector<float> errors_noc_out_curr = disparityErrorsOutlier(D_gt_noc,D_orig,D_ipol,false);//非遮挡区域的比例
      vector<float> errors_occ_out_curr = disparityErrorsOutlier(D_gt_occ,D_orig,D_ipol,false);//所有区域的比例
      vector<float> errors_noc_avg_curr = disparityErrorsAverage(D_gt_noc,D_orig,D_ipol,false);//非遮挡区域的平均误差值
      vector<float> errors_occ_avg_curr = disparityErrorsAverage(D_gt_occ,D_orig,D_ipol,false);//所有区域的平均误差值
      ...
      vector<float> errors_refl_noc_out_curr = disparityErrorsOutlier(D_gt_refl_noc,D_orig,D_ipol,true);//反射面的
      ...
      if (i<194) {
        ...
        system(("copy " + img_src + " " + img_dst).c_str());
      }
  ...
  // write mean
  for (int32_t i=0; i<errors_noc_out[0].size(); i++) { //errors_noc_out[0].size()==11
    fprintf(stats_noc_out_file,"%f ",statMean(errors_noc_out,i));//非遮挡情况，偶数是GT的所有像素，奇数是有效像素，分成误差大于1，2，3，4，5，五种等级的百分比
    fprintf(stats_occ_out_file,"%f ",statMean(errors_occ_out,i));//遮挡情况，，偶数是GT的所有像素，奇数是有效像素，分成误差大于1，2，3，4，5，五种等级的百分比
  }
  ...
  for (int32_t i=0; i<10; i++) {
    fprintf(stats_refl_noc_out_file,"%f ",statWeightedMean(errors_refl_noc_out,i,10+i%2)); //i % 2分成奇数和偶数
...
int32_t main_1 (int32_t argc,char *argv[]) {
...
void ground_truth_false_color()
{
	string gt_noc_dir = "KITTI_training/disp_noc";
	string gt_occ_dir = "KITTI_training/disp_occ";
	string gt_noc_falsecolor_dir = "KITTI_training/disp_noc_falsecolor";

	for (int32_t i = 0; i < 194; i++) { //i<195
									 
		char prefix[256];// file name
		sprintf(prefix, "%06d_10", i);
		printf("Processing: %s.png", prefix);
		// load ground truth disparity maps
		DisparityImage D_gt_noc(gt_noc_dir + "/" + prefix + ".png");
		...
		// compute maximum disparity
		float max_disp = D_gt_occ.maxDisp();
		D_gt_noc.writeColor(gt_noc_falsecolor_dir + "/" + prefix + ".png", max_disp);
	}
}
...
int main_ev()  //main_ev
{
	...
	string result_sha = "results";

	bool success = eval();
	//ground_truth_false_color();
	return 1;
