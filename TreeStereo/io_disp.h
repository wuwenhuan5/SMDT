      ...
      for (int32_t u=1; u<width()-1; u++) {
        if (D_occ.isValid(u,v)) {
          ...
          if (log_colors) {
            ...
            float n_err = min(d_err/3.0,20.0*d_err/d_mag);
            ...
            float d_err = min(fabs(getDisp(u,v)-D_occ.getDisp(u,v)),5.0)/5.0;
            val.red   = (uint8_t)(d_err*255.0);
            val.green = (uint8_t)(d_err*255.0);
            val.blue  = (uint8_t)(d_err*255.0);
			if (!D_noc.isValid(u, v)) { //无效的用蓝色显示
				val.green = 0;
				val.red = 0;
				val.blue = 0;
			}
			if (D_noc.isValid(u, v) && fabs(getDisp(u, v) - D_occ.getDisp(u, v))>3.0) { //让大于3.0像素的用红色显示有效区域
				val.green = 0;
				val.blue = 0;
			}
          }
...
private:
  ...
  void writeDisparityMap (const std::string file_name) {
    ...
    for (int32_t v=0; v<height_; v++) {
      for (int32_t u=0; u<width_; u++) {
        if (isValid(u,v)) image.set_pixel(u,v,(uint16_t)(max(getDisp(u,v)*256.0,1.0)));
  ...
  void writeFalseColors (const std::string file_name, float max_val) {
    ...
    for (int32_t v=0; v<height_; v++) {
      for (int32_t u=0; u<width_; u++) {
        ...
        float val = min(max(getDisp(u,v)/max_val,0.0f),1.0f);
